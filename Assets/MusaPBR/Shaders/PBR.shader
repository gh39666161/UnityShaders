...
Shader "Shader PBR01" {
    ...
    SubShader {
        ...
        Pass {
            ...
            sampler2D _MetalTex; uniform float4 _MetalTex_ST;
            float _Metallic;
            float _Gloss;
            struct VertexInput {
...
//从metallic图上取数据
                fixed4 metalTex = tex2D(_MetalTex, TRANSFORM_TEX(i.uv0, _MetalTex));
                float metallic = metalTex.r;//unity metallic 值，是一个grayscale value ，存在 r 通道
                float perceptualRoughness = 1 - metalTex.a*_Gloss;//unity 用的是smoothness，在metallic map的alpha 通道，这里转换一下
...
////// Specular:
				...
				float3 Kd = DiffuseAndSpecularFromMetallic(albedo, metallic, specularColor, oneMinusReflectivity);
				//————————————————————————————————————————————————————————————————————————————————
